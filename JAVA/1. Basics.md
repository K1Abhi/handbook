...
```
When an expression is entered that doesn't have a named variable, a scratch variable is created so that its value can be referenced later. The following example shows scratch values for an expression and for the results of a method.   
```shell
jshell> 2 + 2
$3 ==> 4
